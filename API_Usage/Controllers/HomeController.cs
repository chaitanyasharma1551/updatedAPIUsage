    ...
    ****/
    ...
    public IActionResult Chart(string symbol)
    {
      ...
      return View(companiesEquities);
        ...
        public IActionResult Dividends(string symbol)
        {
            List<dividends> listOfDividends = GetDividend();
            TempData["Dividends"] = JsonConvert.SerializeObject(listOfDividends);
            return View(listOfDividends);
        }

        public List<dividends> GetDividend()
        {
            // string to specify information to be retrieved from the API
            string IEXTrading_API_PATH = BASE_URL + "/stock/aapl/dividends/1y";
            List<dividends> data = null;
            ...
            if (!charts.Equals(""))
            {
                ...
                data = JsonConvert.DeserializeObject<List<dividends>>(charts);
        ...
        public IActionResult PopulateDividend()
        {
            ...
            List<dividends> dividends = JsonConvert.DeserializeObject<List<dividends>>(TempData["Dividends"].ToString());

            foreach (dividends D in dividends)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.Dividends.Where(c => c.exDate.Equals(D.exDate)).Count() == 0)
                {
                    dbContext.Dividends.Add(D);
                }
            }

            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("Dividends", dividends);
        }

        public IActionResult History(string symbol)
        {
            List<history> HistoryStats = GetHistory();
            TempData["History"] = JsonConvert.SerializeObject(HistoryStats);
            return View(HistoryStats);
        }
        public List<history> GetHistory()
        {
            // string to specify information to be retrieved from the API
            string IEXTrading_API_PATH = BASE_URL + "/stats/historical/daily?date=201605";
            ...
            if (!charts.Equals(""))
            {
                ...
                //JObject result = JsonConvert.DeserializeObject<JObject>(companyList);
                data = JsonConvert.DeserializeObject<List<history>>(charts);
        ...
        public IActionResult PopulateHistory()
        {
            ...
            //  in the symbols method. This example has been structured to demonstrate one way to save object data
            //  and retrieve it later
            List<history> HistoryStats = JsonConvert.DeserializeObject<List<history>>(TempData["History"].ToString());

            foreach (history H in HistoryStats)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.History.Where(c => c.date.Equals(H.date)).Count() == 0)
                {
                    dbContext.History.Add(H);
                }
            }

            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("History", HistoryStats);
        }

        public IActionResult Market(string symbol)
        {
            List<market> MarketStats = GetMarket();
            TempData["Market"] = JsonConvert.SerializeObject(MarketStats);
            return View(MarketStats);
        }
        ...
        public List<market> GetMarket()
        {
            ...
            if (!charts.Equals(""))
            {
                ...
                //JObject result = JsonConvert.DeserializeObject<JObject>(companyList);
                data = JsonConvert.DeserializeObject<List<market>>(charts);
        ...
        public IActionResult PopulateMarket()
        {
            ...
            //  in the symbols method. This example has been structured to demonstrate one way to save object data
            //  and retrieve it later
            List<market> markets = JsonConvert.DeserializeObject<List<market>>(TempData["Market"].ToString());

            foreach (market M in markets)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.Market.Where(c => c.mic.Equals(M.mic)).Count() == 0)
                {
                    dbContext.Market.Add(M);
                }
            }
        ...
        public IActionResult Performance(string symbol)
        {
            List<mktperformance> MarketPerformance = GetPerformance();
            TempData["Performance"] = JsonConvert.SerializeObject(MarketPerformance);
            return View(MarketPerformance);
        }

        public List<mktperformance> GetPerformance()
        {
            // string to specify information to be retrieved from the API
            string IEXTrading_API_PATH = BASE_URL + "/stock/market/sector-performance";
            List<mktperformance> data = null;
            // initialize objects needed to gather data
            string charts = "";
            httpClient.BaseAddress = new Uri(IEXTrading_API_PATH);
            ...
            if (!charts.Equals(""))
            {
                ...
                //JObject result = JsonConvert.DeserializeObject<JObject>(companyList);
                data = JsonConvert.DeserializeObject<List<mktperformance>>(charts);
        ...
        public IActionResult PopulatePerformance()
        {
            ...
            //  in the symbols method. This example has been structured to demonstrate one way to save object data
            //  and retrieve it later
            List<mktperformance> perform = JsonConvert.DeserializeObject<List<mktperformance>>(TempData["Performance"].ToString());

            foreach (mktperformance P in perform)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.NewPerformance.Where(c => c.lastUpdated.Equals(P.lastUpdated)).Count() == 0)
                {
                    dbContext.NewPerformance.Add(P);
                }
            }

            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("Performance", perform);
        }
        ...
        public IActionResult ShortInterest(string symbol)
        {
            List<shortinterest> Interest = GetShortInterest();
            TempData["ShortInterest"] = JsonConvert.SerializeObject(Interest);
        ...
        {
            // string to specify information to be retrieved from the API
            string IEXTrading_API_PATH = BASE_URL + "/stock/market/short-interest/sample";
            List<shortinterest> data = null;
            // initialize objects needed to gather data
            string charts = "";
            httpClient.BaseAddress = new Uri(IEXTrading_API_PATH);
            ...
            if (!charts.Equals(""))
            {
                ...
                //JObject result = JsonConvert.DeserializeObject<JObject>(companyList);
                data = JsonConvert.DeserializeObject<List<shortinterest>>(charts);
        ...
        public IActionResult PopulateInterest()
        {
            // retrieve the companies that were saved in the symbols method
            // saving in TempData is extremely inefficient - the data circles back from the browser
            // better methods would be to serialize to the hard disk, or save directly into the database
            //  in the symbols method. This example has been structured to demonstrate one way to save object data
            //  and retrieve it later
            List<shortinterest> shortinterests = JsonConvert.DeserializeObject<List<shortinterest>>(TempData["ShortInterest"].ToString());

            foreach (shortinterest I in shortinterests)
            {
                //Database will give PK constraint violation error when trying to insert record with existing PK.
                //So add company only if it doesnt exist, check existence using symbol (PK)
                if (dbContext.NewShortInterest.Where(c => c.DaystoCover.Equals(I.DaystoCover)).Count() == 0)
                {
                    dbContext.NewShortInterest.Add(I);
                }
            }

            dbContext.SaveChanges();
            ViewBag.dbSuccessComp = 1;
            return View("ShortInterest", shortinterests);
        }
